...
#' See `system.file("doc", "R_STRATEGUS_COHORT_METHODS_SHELL.md", package = "slashOhdsiStrategusAssistant")`
#' for the installed Cohort Methods workflow guide, and
...
runStrategusCohortMethodsShell <- function(outputDir = "demo-strategus-cohort-methods",
  ...
  collect_recommendation_selection <- function(recommendations, role_label, statement, workflow_type, allow_multiple = FALSE) {
    ...
    selection_results <- lapply(selected_recs, function(selected) {
      phenotype_id <- trimws(as.character(selected$phenotype_id %||% recommendation_identifier(selected)))
      ...
      if (isTRUE(interactive)) {
        ...
        preparation <- .studyAgentSlashPreviewPhenotypeCandidate(client = dialogue_acp_client$client, phenotype_id = phenotype_id, role_label = role_label, workflow_type = workflow_type)
        ...
        if (prepared_direct && identical(tolower(choice), "template")) {
          saved <- .studyAgentSlashPreparePhenotypeConversion(client = dialogue_acp_client$client, phenotype_id = phenotype_id, role_label = role_label, output_dir = output_dir, workflow_type = workflow_type, display = FALSE)
        ...
        if (prepared_direct && !(identical(toupper(choice), "USE") || !nzchar(choice))) return(list(action = "retry"))
        if (prepared_direct) {
          source_rec <- list(phenotype_id = phenotype_id, phenotype_name = as.character((preparation$presentation %||% list())$title %||% selected$phenotype_name %||% phenotype_id), justification = "Imported unchanged from the selected executable OHDSI phenotype.", circe_json = (preparation$source_snapshot %||% list())$source_payload %||% NULL)
          return(list(action = "handled", imported = list(.studyAgentSlashImportAcpCohortDefinition(source_rec, imported_definition_dir))))
        }
      }
      if (isTRUE(recommendation_is_circe_computable(selected))) return(list(action = "handled", imported = list(.studyAgentSlashImportAcpCohortDefinition(selected, imported_definition_dir))))
      if (is.null(dialogue_acp_client$client) && !ensure_workflow_dialogue_client(acpUrl)) stop("ACP bridge unavailable.")
...
")
  ...
  if (isTRUE(skip_intent_split_and_recommendation) && isTRUE(all_cohort_ids_from_function_args) && !isTRUE(direct_acquisition_mode)) {
    ...
    imported_target_selection <- .studyAgentSlashAcquireImportedRoleSelection(
      ...
      prompt_create_computable = function(role_label, allow_multiple = FALSE) { if (is.null(dialogue_acp_client$client) && !ensure_workflow_dialogue_client(acpUrl)) stop("ACP bridge unavailable."); .studyAgentSlashCreateComputableRoleSelection(role_label, switch(role_label, target = targetStatement, comparator = comparatorStatement, outcome = outcomeStatement), dialogue_acp_client$client, output_dir, imported_definition_dir, interactive, readline_with_navigation, is_back_signal, write_json) },
    ...
    if (!is.null(imported_target_selection)) {
      ...
      target_rec <- list(
        selected_ids = as.integer(imported_target_selection$selected_ids),
        selected_source_id = as.character(imported_target$source_id %||% ""),
        selection_source = paste0(target_source_mode, "_import"),
        recommendation_path = json_string_or_null(NULL),
        recommendation_source = paste0(target_source_mode, "_import"),
        used_cached_recommendation = FALSE,
        used_cached_selection = FALSE,
        used_window2 = FALSE,
        used_advice = FALSE,
  ...
  if (isTRUE(skip_intent_split_and_recommendation) && isTRUE(all_cohort_ids_from_function_args) && !isTRUE(direct_acquisition_mode)) {
    ...
    imported_comparator_selection <- .studyAgentSlashAcquireImportedRoleSelection(
      source_mode = comparator_source_mode,
    ...
    if (!is.null(imported_comparator_selection)) {
      if (!identical(imported_comparator_selection$action %||% "", "handled")) next
      imported_comparator <- imported_comparator_selection$imported[[1]]
      comparator_rec <- list(
        selected_ids = as.integer(imported_comparator_selection$selected_ids),
        selected_source_id = as.character(imported_comparator$source_id %||% ""),
        selection_source = paste0(comparator_source_mode, "_import"),
        recommendation_path = json_string_or_null(NULL),
        recommendation_source = paste0(comparator_source_mode, "_import"),
        used_cached_recommendation = FALSE,
        used_cached_selection = FALSE,
        used_window2 = FALSE,
        used_advice = FALSE,
  ...
  if (isTRUE(skip_intent_split_and_recommendation) && isTRUE(all_cohort_ids_from_function_args) && !isTRUE(direct_acquisition_mode)) {
    ...
    if (!is.null(imported_outcome_selection)) {
      ...
      outcome_recs <- list(list(
        selected_ids = as.integer(imported_outcome_selection$selected_ids),
        selected_source_ids = as.character(imported_outcome_selection$selected_source_ids),
        recommendation_path = json_string_or_null(NULL),
        recommendation_source = paste0(outcome_source_mode, "_import"),
        selection_source = paste0(outcome_source_mode, "_import"),
        used_cached_recommendation = FALSE,
        used_cached_selection = FALSE,
        used_window2 = FALSE,
        used_advice = FALSE,
...
", keeper_case_review_state_path))
  ...
  script_header <- c(
    ...
    "# Edit values as needed and run generated scripts in order.",
    if (isTRUE(improvements_applied)) {
      "# NOTE: cohort improvements were already applied in the shell run; 02_apply_improvements.R is a portable record."
    } else {
      "# NOTE: cohort improvements were not applied in the shell run; see 02_apply_improvements.R."
    },
    ""
  )
  ...
  script_07 <- c(
    ...
    ")",
    "",
    "# Characterization module: use the current Characterization settings API directly.",
    "# Strategus 1.5.0 forwards its legacy helper arguments to Characterization 4.0.0,",
    "# so the base Strategus module builder is used with current settings objects.",
    "characterizationTargetIds <- as.numeric(unique(c(target_id, comparator_id)))",
    "characterizationLimitToFirst <- as.numeric(rep(if (isTRUE(getDbDefaults$firstExposureOnly %||% TRUE)) 99999 else 0, length(characterizationTargetIds)))",
    "characterizationPopulationSettings <- lapply(seq_along(characterizationTargetIds), function(i) {",
    "  Characterization::createStudyPopulationSettings(",
    "    targetIds = characterizationTargetIds[[i]],",
    "    limitToFirstInNDays = characterizationLimitToFirst[[i]],",
    "    minPriorObservation = as.numeric(getDbDefaults$washoutPeriod %||% 0)",
    "  )",
    "})",
    "characterizationTargetBaselineSettings <- lapply(characterizationPopulationSettings, function(studyPopulationSettings) {",
    "  Characterization::createTargetBaselineSettings(",
    "    studyPopulationSettings = studyPopulationSettings,",
    "    covariateSettings = FeatureExtraction::createDefaultCovariateSettings()",
    "  )",
    "})",
    "characterizationAnalysis <- Characterization::createCharacterizationSettings(",
    "  timeToEventSettings = NULL,",
    "  dechallengeRechallengeSettings = NULL,",
    "  targetBaselineSettings = characterizationTargetBaselineSettings,",
    "  riskFactorSettings = NULL,",
    "  caseSeriesSettings = NULL",
    ")",
    "characterizationModule <- CharacterizationModule$new()",
    "characterizationModuleSpecifications <- characterizationModule$.__enclos_env__$super$createModuleSpecifications(",
