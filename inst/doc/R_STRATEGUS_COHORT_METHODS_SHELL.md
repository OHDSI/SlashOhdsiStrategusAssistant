...
Usage examples live in `extras/demo_strategus_cohort_method.R` in the source repository.
Package-level installation and quick-start notes live in the repository `README.md`.

For an installed package, locate this guide with
`system.file("doc", "R_STRATEGUS_COHORT_METHODS_SHELL.md", package = "slashOhdsiStrategusAssistant")`.
...
3. Role-specific cohort acquisition for target, comparator, and outcome cohorts using an ACP recommendation when AI is enabled, a Phenotype Library definition, an existing cohort-definition database, a local cohort JSON file, or a directory of local cohort JSON files. ACP is not required for the other providers.
